    ...
    try {
        ...
        if (isEditing) {
            ...
            if (docSnap.exists) {
                ...
                // Iniciar transacción para manejar la actualización de puntos
                await db.runTransaction(async transaction => {
                    // 1. Restar puntos actuales
                    // Sede
                    if (wogActual.sede) {
                    ...
                    if (wogActual.asadores && wogActual.asadores.length > 0) {
                        const puntoPorAsador = 1 / wogActual.asadores.length;
                    ...
                    if (wogActual.comprasCompartidas && wogActual.comprasCompartidas.length > 0) {
                        const puntoPorCompra = 1 / wogActual.comprasCompartidas.length;
                    ...
                    // 3. Sumar nuevos puntos
                    // Nueva sede
                    if (wogData.sede) {
                        const nuevaSedeRef = db.collection(COLECCION_PARTICIPANTES).doc(wogData.sede);
                    ...
                    if (wogData.asadores && wogData.asadores.length > 0) {
                        const nuevoPuntoPorAsador = 1 / wogData.asadores.length;
                        ...
                        for (const asadorId of wogData.asadores) {
                            ...
                            transaction.update(nuevoAsadorRef, {
                                puntos_asador: firebase.firestore.FieldValue.increment(nuevoPuntoPorAsador)
                            });
                        }
                    }
                    ...
                    if (wogData.comprasCompartidas && wogData.comprasCompartidas.length > 0) {
                        ...
                        for (const compraId of wogData.comprasCompartidas) {
                            ...
                            transaction.update(nuevaCompraRef, {
                                puntos_compras: firebase.firestore.FieldValue.increment(nuevoPuntoPorCompra)
                            });
                        }
