...
function openTab(tabId) {
    ...
    if (tabElement) {
        ...
        // Activar el botón correspondiente buscando el que tiene el onclick con este tabId
        document.querySelectorAll('.tab-button').forEach(button => {
    ...
    // Esta línea es importante para evitar comportamiento indeseado del navegador
    return false;
}
        ...
        setTimeout(async () => {
            try {
                ...
                const fechaFormateada = [
                    fechaWog.getFullYear(),
                    String(fechaWog.getMonth() + 1).padStart(2, '0'),
                    String(fechaWog.getDate()).padStart(2, '0')
                ].join('-');
...
function mostrarNotasWogAntiguo(fecha, notas) {
    const modalNotas = document.getElementById('modal-notas');
...
window.mostrarNotasWogAntiguo = mostrarNotasWogAntiguo;
